...
def load_fake_images(args):
    all_paths = []
    for i in range(5):
        print('load fake images valid_%d' % i)
        valid_path = os.path.join(args.data_dir, 'output', args.netG, 'valid/valid_%d/single' % i)
        img_paths = glob.glob(os.path.join(valid_path,'*.png'))
        all_paths.extend(img_paths)
    imgs = load_np_images(all_paths)
    return imgs

    """
    epoch_path = os.path.join(args.data_dir, "output", args.netG, "valid", args.model_epoch, args.split) ##
    if not os.path.exists(epoch_path):
        print('this path not exists.', epoch_path)
    stage_all_paths = {0:[]}
...
def load_filenames(args):
    with open(os.path.join(args.data_dir, 'test/filenames.pickle'), "rb") as f:
...
if __name__=="__main__":
    ...
    parser.add_argument('--data_dir', type=str, default="/data/unagi0/ktokitake/encdecmodel/birds")
    parser.add_argument("--netG",type=str, default="birds_attn2_2020_08_03_14_39_16")
    parser.add_argument("--m_epoch", type=str, dest="model_epoch", default="600")
    ...
    real_images = load_real_images(args, filenames)
    fake_images = load_fake_images(args)
    ...
    print("now calcurating fake")
    mu_s1, cov_s1 = calc_statistics(fake_images, model, args.gpus)
    fid_s1 = calc_fid_score(mu_real, cov_real, mu_s1, cov_s1)
    print("FID_score : %.3f" % fid_s1)
