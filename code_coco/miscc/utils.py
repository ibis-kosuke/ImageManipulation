...
def build_images(real_imgs, fake_imgs, caption, w_caption, ixtoword):
    set_imgs = []
    for i in range(caption.shape[0]):
        font = ImageFont.truetype('./FreeMono.ttf', 20)
        real_canv = Image.new('RGB', (256,256), (255,255,255))
        real_draw = ImageDraw.Draw(real_canv)
        fake_canv = Image.new('RGB', (256,256), (255,255,255))
        fake_draw = ImageDraw.Draw(fake_canv)
        sent = []
        for j in range(len(caption[i])):    
            if(caption[i][j]==0):
                break
            word = ixtoword[caption[i][j].item()].encode('ascii', 'ignore').decode('ascii')
            if(j%2==1):
                word = word+'\n'
            sent.append(word)
        real_sent = ' '.join(sent)
        sent = []
        for j in range(len(w_caption[i])):
            if(w_caption[i][j]==0):
                break
            word = ixtoword[w_caption[i][j].item()].encode('ascii', 'ignore').decode('ascii')
            if(j%2==1):
                word = word+'\n'
            sent.append(word)
        fake_sent = ' '.join(sent)
        real_draw.text((0,0), real_sent, font=font, fill=(0,0,0))
        fake_draw.text((0,0), fake_sent, font=font, fill=(0,0,0))

        real_canv_np = np.asarray(real_canv)
        fake_canv_np = np.asarray(fake_canv)

        real_img = real_imgs[i].add_(1).div_(2).mul_(255)
        real_img = real_img.cpu().numpy().astype(np.uint8)
        real_img = np.transpose(real_img, (1,2,0))

        fake_img = fake_imgs[i].add_(1).div_(2).mul_(255)
        fake_img = fake_img.detach().cpu().numpy().astype(np.uint8)
        fake_img = np.transpose(fake_img, (1,2,0))
        
        one_set = np.concatenate([real_canv_np, real_img, fake_canv_np, fake_img], axis=1)
        set_imgs.append(one_set)

    final_set = []
    for i in range(1, caption.shape[0]//2+1):
        row_img = np.concatenate(set_imgs[(i-1)*2:i*2], axis=1)
        final_set.append(row_img)
    final_set = np.concatenate(final_set, axis=0)
