...
def prepare_data(data):
    imgs, captions, captions_lens, full_caps, class_ids, keys, wrong_caps, \
    ...
    full_caps = full_caps[sorted_cap_indices].squeeze()
    ...
    return [real_imgs, captions, sorted_cap_lens, full_caps,
...
class TextDataset(data.Dataset):
    ...
    def get_caption(self, sent_ix):
        ...
        full_cap = np.zeros((cfg.TEXT.WORDS_NUM2, 1))
        full_cap[:num_words, 0] = sent_caption
    ...
    def __getitem__(self, index):
        ...
        ##img_name = '%s/images/%s.jpg' % (data_dir, key) #for CUB
        img_name = '%s/%s/%s2014/%s.jpg' %(data_dir, self.split, self.split_, key) # for coco
        ...
        caps, cap_len, full_cap = self.get_caption(new_sent_ix)

        ###
        wrong_idx = random.randint(0, len(self.filenames))
        wrong_new_sent_ix = wrong_idx * self.embeddings_num + sent_ix
        wrong_caps, wrong_cap_len, w_full_cap = self.get_caption(wrong_new_sent_ix)
        wrong_cls_id = self.class_id[wrong_idx]
        ###

        return imgs, caps, cap_len, full_cap, cls_id, key, wrong_caps, \
