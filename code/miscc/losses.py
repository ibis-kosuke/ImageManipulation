...
# ################## Loss for G and Ds ##############################
def discriminator_loss(netD, real_imgs, fake_imgs, sent_emb, w_sent_emb,
    ...
    cond_fake_logits = netD.COND_DNET(fake_features, w_sent_emb)
...
def generator_loss(netD, fake_img, real_img, w_img, real_labels, w_sent_emb, VGG, gpus):
    ...
    cond_logits = netD.COND_DNET(features, w_sent_emb)
    ...
    feature_loss = F.l1_loss(fake_img, real_img)
    logs += 'feature_loss: %.2f ' % feature_loss
    errG_total += feature_loss


    real_enc_features = nn.parallel.data_parallel( VGG, (real_img), gpus)
    fake_enc_features = nn.parallel.data_parallel( VGG, (fake_img), gpus)
    w_enc_features = nn.parallel.data_parallel( VGG, (w_img), gpus)

    hidden_feature_loss_real = F.mse_loss(real_enc_features[1], fake_enc_features[1])
    hidden_feature_loss_w = F.mse_loss(w_enc_features[4], fake_enc_features[4])
    logs += 'hidden_feature_loss_real: %.2f ' % (hidden_feature_loss_real)
    logs += 'hidden_feature_loss_w: %.2f ' %(hidden_feature_loss_w)
    errG_total += hidden_feature_loss_real
    errG_total += hidden_feature_loss_w
